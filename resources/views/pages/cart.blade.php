...
@if(isset($subtotalAmount))
  ...
  <form class="mb-32" action="{{route('post.order')}}" method="POST">
    @csrf

    @foreach($datas AS $index => $item)
      <input type="hidden" name="products[{{ $index }}][product_id]" value="{{ $item->product_id }}">
      <input type="hidden" name="products[{{ $index }}][quantity]" value="{{ $item->quantity }}">
      <input type="hidden" name="products[{{ $index }}][productprice]" value="{{ $item->productprice}}">
    @endforeach

    <input type="hidden" value='{{$subtotalAmount}}' name='total_amount'>
    <input type="hidden" value={{ Auth::user()->id}} name='user_id'>

    <div class="bg-amber-400 flex justify-center mx-10 p-1 rounded">
      <button type='submit'>passer la commande ({{$cartCount}} articles)</button>
    </div>
  </form>
@endif
